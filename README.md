<div align="center"><img src="assets/banner.png" alt="ALPHA NIXUS — 24/7 Minecraft AFK Bot" width="100%"><br>
...
cp .env.example .env
Ab .env kholo aur ye poora block paste kar do
(ya hosting panel ke Env → Raw .env section me):
...
🔴 Sirf ye 3 lines badalni hain
Variable	Kya daalo	Example
MC_HOST	Server ka address — port ke bina	play.yourserver.net
MC_PORT	Server ka port	25565
AUTH_PASSWORD	Bot ka password	ChangeThisPassword123
⚠️ Agar server ka address play.example.net:30125 jaisa hai, to usse do hisso me todo:
MC_HOST=play.example.net aur MC_PORT=30125 — dono alag lines me, : mat likhna.

🎛️ AUTH_MODE kya chuno?
Value	Kab use karo
auto ⭐	Default — server ka message padh kar khud decide karta hai
login	Account pehle se registered hai (owner ne authme register chala diya)
register	Sirf naya account banana hai
none	Server par auth plugin hai hi nahi, ya bot AuthMe ki UnrestrictedName list me hai
📨 Discord webhook (optional par recommended)
Discord channel → Settings ⚙️ → Integrations → Webhooks → New Webhook
Copy Webhook URL → DISCORD_WEBHOOK me paste karo
Nahi chahiye? Line hata do — bot bilkul theek chalega, logs console me aayenge
🔒 Kabhi bhi apna asli password ya webhook URL GitHub par commit mat karna. .env already .gitignore me hai ✅
...
🌍 Deploy karna hai? → docs/HOSTING.md me Oracle Cloud, bot panels, Fly.io, Docker aur PM2 — sab ke step-by-step guides hain.
...
AUTH_DELAY_MS	4000	Join ke baad auth delay
AUTH_RETRY_MS	6000	Retry interval
AUTH_MAX_RETRY	7	Max attempts (AuthMe ka 60 s window cover karta hai)
LOG_SERVER_CHAT	true	Server ke messages console me mirror karo (debugging)
</details><details> <summary><b>🪪 Usernames &amp; Rotation</b></summary><br>
Variable	Default	Description
...
</details><details> <summary><b>🎭 Human Behaviour</b></summary><br>
Variable	Default	Description
...
</details><details> <summary><b>📨 Discord Logging</b></summary><br>
Variable	Default	Description
...
</details><details> <summary><b>🔧 Runtime</b></summary><br>
Variable	Default	Description
...
└── .env.example
🛠️ Troubleshooting
<details> <summary><b>Bot connect nahi ho raha</b></summary><br>
Error	Wajah	Fix
...
</details><details> <summary><b>Join karte hi kick ho jata hai</b></summary><br>
...
</details><details> <summary><b>Bot hil nahi raha</b></summary><br>
HUMAN_MODE=true set hai? Console me warning aayegi agar nahi.
Console me Movement engine STARTED dikha? Nahi to spawn event nahi aaya — 20 s ka fallback apne aap chalega.
AuthMe login pending to nahi? Login hone tak server player ko freeze rakhta hai.
Bot AFK-cage / spawn protection me to nahi? Jump phir bhi hoga, position nahi badlegi.
</details><details> <summary><b>Login / register fail — "Login timeout exceeded" kick</b></summary><br>
Console me SERVER> lines dekho — wahan auth plugin ka exact message hoga.

Sabse pakka fix (server owner ho to): server console me har bot name manually register kar do —

text

authme register ALPHA_NIXUS1 YourPassword123
authme register ALPHA_NIXUS2 YourPassword123
authme register ALPHA_NIXUS3 YourPassword123
authme register ALPHA_NIXUS4 YourPassword123
authme register ALPHA_NIXUS5 YourPassword123
Phir env me AUTH_MODE=login aur wahi AUTH_PASSWORD set karo.

Baaki wajah:

Console me dikhega	Fix
PERMISSION MISSING	lp group default permission set authme.player.login true
PASSWORD GALAT HAI	authme password <name> <new> ya sahi AUTH_PASSWORD
Password policy reject	Lamba password (8+ chars, letters + numbers)
Captcha required	AuthMe config me captcha off, ya manual register
</details><details> <summary><b>Storage / memory full</b></summary><br>
Storage: pehli boot par auto-cleanup chalta hai (426 MB → 15 MB). node_modules delete karke restart karo.
Memory: heap 200 MB par capped hai; RSS limit cross kare to bot khud session refresh karta hai.
Aur kam chahiye? KEEP_MC_VERSIONS=1.12,1.12.1,1.12.2 set karo.
</details><details> <summary><b>Discord logs nahi aa rahe</b></summary><br>
...
Koi idea hai? Feature request kholo 💡

🤝 Contributing
Contributions welcome! CONTRIBUTING.md padho.
...
📄 License
MIT © 2026 Deep Mallick (AKA Mr.Mallick)
